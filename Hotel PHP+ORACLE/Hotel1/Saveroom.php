...
class room extends connect
{
	...
	public function save()
	{
                if($this->db_found)
		{
				...
				if($f==0)
				{
		                        $sql="insert into room values('$_POST[t1]','$_POST[t2]','$_POST[t3]','$_POST[t4]','$_POST[t5]')";
		                        mysqli_query($this->db_found,$sql);
		                        echo "<script>alert('Record Saved')</script>";
                                }
				else
					echo ("<script>alert('Room id already exist')</script>");
		}//End of  save
	}
...
echo"
    	<form name=f method=post action=Saveroom.php>
		<h1>Room Form<h1>
		<label>Room Id</label>
